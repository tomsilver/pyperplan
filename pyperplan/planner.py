...
def _search(task, search, heuristic, rng, max_nodes_created, use_preferred_ops=False):
    logging.info(f"Search start: {task.name}")
    if heuristic:
        if use_preferred_ops:
            solution, metrics = search(task, heuristic, rng, max_nodes_created,
                                       use_preferred_ops)
        else:
            solution, metrics = search(task, heuristic, rng, max_nodes_created)
    else:
        solution, metrics = search(task, rng, max_nodes_created)
...
def search_plan(
    domain_file, problem_file, search, heuristic_class, rng, max_nodes_created,
    ...
    if use_preferred_ops and isinstance(heuristic, heuristics.hFFHeuristic):
        solution, metrics = _search(task, search, heuristic, rng,
                                    max_nodes_created, use_preferred_ops=True)
    else:
        solution, metrics = _search(task, search, heuristic, rng, max_nodes_created)
